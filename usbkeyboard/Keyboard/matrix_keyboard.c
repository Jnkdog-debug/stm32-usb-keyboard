...
static const uint16_t row_pins[KEYBOARD_ROWS] = {ROW_PIN_0, ROW_PIN_1,
                                                 ROW_PIN_2};
static const uint16_t col_pins[KEYBOARD_COLS] = {COL_PIN_0, COL_PIN_1,
                                                 COL_PIN_2};
...
void Matrix_Keyboard_Scan(void) {
  ...
  for (uint8_t row = 0; row < KEYBOARD_ROWS; row++) {
    ...
    for (uint8_t i = 0; i < KEYBOARD_ROWS; i++) {
      if (i == row) {
        HAL_GPIO_WritePin(ROW_PORT, row_pins[i],
                          GPIO_PIN_RESET); // LOW = active
      } else {
        HAL_GPIO_WritePin(ROW_PORT, row_pins[i],
                          GPIO_PIN_SET); // HIGH = inactive
      }
    }
    ...
    for (col = 0; col < KEYBOARD_COLS; col++) {
      ...
      /* With pull-up: GPIO_PIN_RESET (0) = pressed, GPIO_PIN_SET (1) = not
       * pressed */
